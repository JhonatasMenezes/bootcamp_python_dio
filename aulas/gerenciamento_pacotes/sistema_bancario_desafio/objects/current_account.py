...
class CurrentAccount(Account):
    def __init__(
        self, account_id: int, customer: Customer, limit=1000, cash_out_lmits=3
    ) -> None:
    ...
    def cash_out(self, value: float) -> bool:
        number_cash_outs = len(
            [
                ...
                if transaction["transaction_type"] == GetMoney.__name__
            ]
        )
    ...
    def __repr__(self) -> str:
        return f"<{self.__class__.__name__}: ('{self.agency}' '{self.account_id}' '{self.customer.name}')>"
